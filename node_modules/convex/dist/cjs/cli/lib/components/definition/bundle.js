  ...
  return {
    ...
    async setup(build) {
      build.onResolve({ filter: /.*component.config.*/ }, async (args) => {
        verbose && (0, import_context.logMessage)(ctx, "esbuild resolving import:", args);
        if (args.namespace !== "file") {
          verbose && (0, import_context.logMessage)(ctx, "  Not a file.");
          return;
        }
        if (args.kind === "entry-point") {
          verbose && (0, import_context.logMessage)(ctx, "  -> Top-level entry-point.");
        ...
        for (const candidate of candidates) {
          try {
            const require2 = (0, import_module.createRequire)(args.resolveDir);
            resolvedPath = require2.resolve(candidate, {
        ...
        if (resolvedPath === void 0) {
          verbose && (0, import_context.logMessage)(ctx, `  -> ${args.path} not found.`);
          return;
        }
        ...
        if (!imported) {
          ...
          if (isComponent.kind !== "ok") {
            verbose && (0, import_context.logMessage)(ctx, "  -> Not a component:", isComponent);
            return;
          }
          imported = isComponent.component;
          components.set(resolvedPath, imported);
        }
        verbose && (0, import_context.logMessage)(ctx, "  -> Component import! Recording it.", args.path);
        if (mode === "discover") {
          return {
            path: resolvedPath
          };
...
function hackyMapping(componentPath) {
...
async function componentGraph(ctx, absWorkingDir, rootComponentDirectory, verbose = true) {
  let result;
  try {
    ...
    (0, import_context.logError)(ctx, `esbuild failed: ${err}`);
    ...
    for (const importPath of componentImports.map((dep) => dep.path)) {
      ...
      if (!imported) {
        (0, import_context.logError)(
          ctx,
          `Didn't find ${import_path.default.resolve(importPath)} in ${[...componentsByAbsPath.keys()].toString()}`
        );
...
async function bundleDefinitions(ctx, absWorkingDir, dependencyGraph, rootComponentDirectory, componentDirectories, verbose = false) {
  let result;
  try {
    result = await import_esbuild.default.build({
      absWorkingDir,
      ...
      sourcemap: false,
      // we're just building a deps map
      ...SHARED_ESBUILD_OPTIONS
    });
    await registerEsbuildReads(ctx, absWorkingDir, result.metafile);
  } catch (err) {
  ...
  if (appBundles.length !== 1) {
    (0, import_context.logError)(ctx, "found wrong number of app bundles");
    return await ctx.crash(1, "fatal");
  }
...
async function bundleImplementations(ctx, rootComponentDirectory, componentDirectories, verbose = false) {
  ...
  for (const directory of [rootComponentDirectory, ...componentDirectories]) {
    ...
    const schema = (await (0, import_bundler.bundleSchema)(ctx, resolvedPath))[0] || null;
    const entryPoints = await (0, import_bundler.entryPointsByEnvironment)(
      ctx,
      resolvedPath,
      verbose
    );
    const convexResult = await (0, import_bundler.bundle)(ctx, resolvedPath, entryPoints.isolate, true, "browser");
    if (convexResult.externalDependencies.size !== 0) {
      (0, import_context.logError)(ctx, "external dependencies not supported");
      return await ctx.crash(1, "fatal");
    }
  ...
  if (!appImplementation) {
    (0, import_context.logError)(ctx, "No app implementation found");
    return await ctx.crash(1, "fatal");
  }
...
async function registerEsbuildReads(ctx, absWorkingDir, metafile) {
  for (const [relPath, input] of Object.entries(metafile.inputs)) {
    if (
      ...
      import_path.default.basename(relPath).includes("app.config") || import_path.default.basename(relPath).includes("component.config") || // TODO: esbuild outputs paths prefixed with "(disabled)" when bundling our internal
