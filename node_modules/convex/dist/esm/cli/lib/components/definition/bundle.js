  ...
  return {
    ...
    async setup(build) {
      build.onResolve({ filter: /.*component.config.*/ }, async (args) => {
        ...
        verbose && logMessage(ctx, "  -> Component import! Recording it.", args.path);
        if (mode === "discover") {
          return {
            path: resolvedPath
          };
...
export async function componentGraph(ctx, absWorkingDir, rootComponentDirectory, verbose = true) {
  let result;
  try {
...
export async function bundleDefinitions(ctx, absWorkingDir, dependencyGraph, rootComponentDirectory, componentDirectories, verbose = false) {
  let result;
  try {
    result = await esbuild.build({
      absWorkingDir,
...
export async function bundleImplementations(ctx, rootComponentDirectory, componentDirectories, verbose = false) {
  ...
  for (const directory of [rootComponentDirectory, ...componentDirectories]) {
    ...
    const convexResult = await bundle(ctx, resolvedPath, entryPoints.isolate, true, "browser");
...
async function registerEsbuildReads(ctx, absWorkingDir, metafile) {
  for (const [relPath, input] of Object.entries(metafile.inputs)) {
    if (
      ...
      path.basename(relPath).includes("app.config") || path.basename(relPath).includes("component.config") || // TODO: esbuild outputs paths prefixed with "(disabled)" when bundling our internal
